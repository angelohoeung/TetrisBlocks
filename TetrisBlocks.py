...
def drawAll(block, grid, title, screen, colours, blockX, blockY, blockSize, score, nBlock, lose):
    screen.blit(title, (0, 0))  # title screen

    for x in range(len(block)):
        for y in range(len(block[0])):
            if block[x][y] > 0:  # fills in part of block if it's not 0
                screen.blit(colours[block[x][y] - 1], (blockX + x *
                            blockSize, blockY + y*blockSize, blockSize, blockSize))
                # blits the block with its corresponding colour

    for x in range(len(grid)):
        for y in range(len(grid[0])):
            if grid[x][y] > 0:
                # fills in all elements in grid more than 0 and finds those block's colours in the colour list
                screen.blit(colours[grid[x][y] - 1], (512 - 5*blockSize +
                            y*blockSize, 100 + x*blockSize, blockSize, blockSize))

    for x in range(11):
        for y in range(21):
            draw.line(screen, (214, 212, 211), (512 - 5*blockSize + x*blockSize, 100),
                      (512 - 5*blockSize + x*blockSize, 100 + 20*blockSize))  # creates the grid lines
            draw.line(screen, (214, 212, 211), (512 - 5*blockSize, 100 +
                      y*blockSize), (512 + 5*blockSize, 100 + y*blockSize))
    ...
    high = helvetica.render(
        "High Score: " + str(readhighscore()), True, (255, 255, 255))
    screen.blit(high, (680, 115))
    screen.blit(points, (680, 145))
    # text displays score, highscore, and next
    screen.blit(nextBlock, (680, 260))

    for x in range(len(nBlock)):
        for y in range(len(nBlock[0])):
            if nBlock[x][y] > 0:
                # displays the next block on the right of the grid
                screen.blit(colours[nBlock[x][y] - 1], (690 + x *
                            blockSize, 300 + y*blockSize, blockSize, blockSize))
    draw.rect(screen, (255, 255, 255), (688, 298, blockSize*2 + 2,
              blockSize*4 + 2), 2)  # rect for where next block is
...
def collision(block, blockX, blockY, blockSize, grid, bottom):
        ...
        for y in range(len(block[0])):
            if block[x][y] > 0:
                if blockY >= bottom:  # collision occurs when block is at bottom
                    ...
                    # this calculates the current location of the block in the grid. if there's a block below, collision occurs
                    if grid[int((blockY - 100)/blockSize + y + 1)][int((blockX - (512 - 5*blockSize))/blockSize + x)] > 0:
                        collide = True
    ...
    if collide:
        for x in range(len(block)):
            for y in range(len(block[0])):
                if block[x][y] > 0:
                    grid[int((blockY - 100)/blockSize + y)][int((blockX - (512 - 5*blockSize)) /
                                                                blockSize + x)] = block[x][y]  # the block stops falling when colliding
...
def LRcollide(block, blockX, blockY, blockSize, grid):
    for x in range(len(block)):
        for y in range(len(block[0])):
            if block[x][y] > 0:
                try:  # Tries to find if there's a block left or right of current block unless there is an index error, then there's no LR block collision
    ...
    for x in range(len(rotate(block))):
        for y in range(len(rotate(block)[0])):
            if rotate(block)[x][y] > 0:
                try:
                    # if the location that the rotation will be is already occupied, there is a collision
                    if grid[int((blockY - 100)/blockSize + y)][int((blockX - (512 - 5*blockSize))/blockSize + x)] > 0:
                        return True
...
def main():
    ...
    # --------------------------IMAGES AND MUSIC-------------------------------------
    green = transform.scale(image.load(
        "images/green.png"), (blockSize, blockSize))
    blue = transform.scale(image.load("images/blue.png"),
                           (blockSize, blockSize))
    cyan = transform.scale(image.load("images/cyan.png"),
                           (blockSize, blockSize))
    purple = transform.scale(image.load(
        "images/purple.png"), (blockSize, blockSize))
    orange = transform.scale(image.load(
        "images/orange.png"), (blockSize, blockSize))
    red = transform.scale(image.load("images/red.png"), (blockSize, blockSize))
    yellow = transform.scale(image.load(
        "images/yellow.png"), (blockSize, blockSize))
    ...
    colours = [blue, orange, purple, yellow, cyan,
               green, red]  # list of each colour block
    ...
    while running:
        ...
        # the grid's bottom minus the block's height
        bottom = 100 + 20*blockSize - blockSize*len(block[0])
        if state:
            ...
            if pressed:  # starts game when anything pressed
                ...
                if delayLR == 5:  # delays the left and right movement
                    if not collision(block, blockX, blockY, blockSize, grid, bottom):
                        if keys[K_RIGHT]:
                            # allows right movement when no collision on right
                            if LRcollide(block, blockX, blockY, blockSize, grid) != 2:
                                # allows movement in increments and stops movement at right border
                                blockX = min(
                                    blockX + blockSize, 512 + 5*blockSize - blockSize*len(block))
                        if keys[K_LEFT]:
                            # allows left movement when no collision on left
                            if LRcollide(block, blockX, blockY, blockSize, grid) != 1:
                                # allows movement in increments and stops movement at left border
                                blockX = max(blockX - blockSize,
                                             512 - 5*blockSize)
                        delayLR = 0
                ...
                if collision(block, blockX, blockY, blockSize, grid, bottom):
                    block = nBlock
                    # the block equals next block and next block is randomized
                    nBlock = newBlock(blocks)
                ...
                drawAll(block, grid, title, screen, colours, blockX,
                        blockY, blockSize, score, nBlock, lose)
                myClock.tick(60)
